...
# Directory Setup for QLoRA Training Logs
LOGS_DIR = "logs_qlora_general"  # Directory where QLoRA training logs are stored
RESULTS_FILE_PATTERN = "training_results_qlora_*.json"  # Pattern to match multiple training result files
...
def load_results():
    ...
    - Searches for log files matching `training_results_qlora_*.json`
    ...
    log_files = sorted([
        ...
        if f.startswith("training_results_qlora") and f.endswith(".json")
    ])
...
def plot_training_loss(results):
    ...
    plt.title("QLoRA Training Loss Comparison Across Datasets")  # Plot title
