...
# Directory for LoRA Training Logs
LOGS_DIR = "logs_lora_general"
RESULTS_FILE_PATTERN = "training_results_lora_*.json"  # Match multiple runs

# Load Training Results
def load_results():
    """Load results from multiple JSON files."""
    results = {}
    log_files = sorted([f for f in os.listdir(LOGS_DIR) if f.startswith("training_results_lora") and f.endswith(".json")])
...
def plot_training_loss(results):
    ...
    plt.ylabel("Loss")
    plt.title("LoRA Training Loss Comparison Across Datasets")
    plt.legend()
